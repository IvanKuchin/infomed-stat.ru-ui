...
export default class OddsRatio {
    ...
    _GetTableBody(matrix, callback_item_value) {
        ...
        for (let i = 0; i < matrix.length; i++) {
            ...
            let ds_id = (_b = (_a = this._datasets[i]) === null || _a === void 0 ? void 0 : _a.parent_id) !== null && _b !== void 0 ? _b : i;
    ...
    _DrawGUITable(table_selector, matrix, callback_item_value, titles) {
        const el = document.querySelector(table_selector);
        el.innerHTML = "";
        el.appendChild(this._GetTableDOM(matrix, titles !== null && titles !== void 0 ? titles : this._HumanizeTitles(this.calc_odds.GetMonths()), callback_item_value));
    }
    _DrawMedianGUITable(table_title, matrix, callback_item_value) {
        const el = document.querySelector(`[ms-group="${this.id}"] [${table_title}]`);
        el.innerHTML = "";
    ...
    _Draw_OR_GUITables(table_title, data) {
        ...
        const or = data.or;
        const container = document.querySelector(`[or-group="${this.id}"] [${table_title}]`);
        container.innerHTML = "";
        for (let i = 0; i < months.length; i++) {
    ...
    _AddExplanations(explanation_id, text) {
        const el = document.querySelector(`[or-group="${this.id}"] [${explanation_id}]`);
        el.innerHTML = text;
    }
    _GetProb(item) {
