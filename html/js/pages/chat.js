...
chat = (function () {
	...
	var WSHandling = function () {
		try {
			ws = new WebSocket(
				"ws://" + window.location.hostname + ":7681", "text-message-protocol");
		...
		if ((messageObj.fromID == myUserID) && (messageObj.toID == activeUserID)) {
			...
			if (appendFlag) {
				...
				if (animateFlag) {
					...
					divMessageFromMe.animate({ "top": "0px" }, {
		...
		if ((messageObj.fromID == activeUserID) && (messageObj.toID == myUserID)) {
			...
			if (appendFlag) {
				...
				if (animateFlag) {
					...
					// divMessageFromMe.css("right", "-100%"); 
					ScrollMessageListToLastMessage();
					divMessageFromFriend.animate({ "opacity": "1", "top": "0px" }, {
						duration: 500,
						complete: function () {
							ScrollMessageListToLastMessage();
						}
					});
					divAvatarFromFriend.animate({ "opacity": "1", "left": "0px" }, {
						duration: 500,
						complete: function () {
							ScrollMessageListToLastMessage();
						}
